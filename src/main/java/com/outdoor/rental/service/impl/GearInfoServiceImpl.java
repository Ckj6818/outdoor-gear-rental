...
public class GearInfoServiceImpl implements GearInfoService {
    ...
    /** 去除空白参数，避免无效条件进入 SQL */
    private void normalizeQuery(GearInfoQueryDTO query) {
        if (query.getKeyword() != null) {
            String keyword = query.getKeyword().trim();
            query.setKeyword(keyword.isEmpty() ? null : keyword);
        }
        if (query.getGearName() != null) {
            String gearName = query.getGearName().trim();
            query.setGearName(gearName.isEmpty() ? null : gearName);
        }
        if (query.getCategory() != null) {
            String category = query.getCategory().trim();
            query.setCategory(category.isEmpty() ? null : category);
        }
        if (query.getBrand() != null) {
            String brand = query.getBrand().trim();
            query.setBrand(brand.isEmpty() ? null : brand);
        }
